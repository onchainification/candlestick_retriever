...
def gather_new_candles(base, quote, last_timestamp, interval='1m', n=0, n_count=0):
    ...
    bar = None
    if last_timestamp == 0:
        print(f'{datetime.now()} {n:04d}/{n_count} Starting from the beginning (no last known timestamp)')
    else:
        print(f'{datetime.now()} {n:04d}/{n_count} Starting from last known timestamp ({datetime.fromtimestamp(last_timestamp / 1000)})')
    while previous_timestamp != last_timestamp:
        ...
        if first_read:
            ...
            print(f'{datetime.now()} {n:04d}/{n_count} Fetching all available data from last {missing_data_timedelta} (max {total_minutes_of_data} candles)')
            if IN_PYCHARM:
                time.sleep(0.2)
            first_read = False
...
def all_candles_to_parquet(base, quote, interval='1m', n=0, n_count=0):
    ...
    new_candle_batches = gather_new_candles(base, quote, last_timestamp, interval, n, n_count)
...
def main():
    ...
    for n, pair in enumerate(all_pairs, 1):
        ...
        if new_lines > 0:
            print(f'{datetime.now()} {n:04d}/{n_count} Wrote {new_lines} new candles to file for {base}-{quote}')
        else:
            print(f'{datetime.now()} {n:04d}/{n_count} Already up to date with {base}-{quote}')
